...
export default function CheckTaskModal({
  task,
  onClose,
  onConfirm,
}: CheckTaskModalProps) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm">
      <div className="bg-white/20 backdrop-blur-xl border border-white/40 shadow-2xl rounded-3xl p-6 w-full max-w-sm flex flex-col gap-4 scale-100 transition-all">
        <h2 className="text-white text-xl font-bold text-shadow-glossy flex items-center gap-2">
          <span>⚠️</span> Atualizar Tarefa
        </h2>

        <p className="text-white/90 drop-shadow-sm leading-relaxed">
          Deseja marcar a tarefa como{" "}
          {task.checked ? "não concluída" : "concluída"}?
        </p>
        ...
        <div className="flex justify-end gap-3 mt-4">
          ...
          <button
            onClick={onConfirm}
            className="px-4 py-2 rounded-full border border-red-300/60 bg-gradient-to-b from-sky-400 to-sky-600 shadow-glossy text-white font-medium hover:brightness-110 active:scale-95 transition-all text-shadow-glossy"
          >
            Sim, {task.checked ? "Desconcluír" : "Concluír"}
          </button>
        </div>
      </div>
    </div>
